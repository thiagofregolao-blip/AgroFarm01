...
export default function ChatBot({ userRole }: { userRole?: string }) {
    ...
    const isAdmin = userRole === "administrador" || userRole === "admin_agricultor";
    ...
    return (
        <>
            ...
            <button
                ...
                style={{ background: "linear-gradient(135deg, #00450d, #1b5e20)" }}
            ...
            {open && (
                <div className="fixed bottom-24 right-6 z-[998] w-[380px] max-w-[calc(100vw-2rem)] h-[520px] max-h-[calc(100vh-8rem)] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden"
                    ...
                    {/* Header */}
                    <div className="px-4 py-3 flex items-center gap-3" style={{ background: "linear-gradient(135deg, #00450d, #1b5e20)" }}>
                        <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center">
                            <Bot className="w-5 h-5 text-white" />
                        </div>
                        <div className="flex-1">
                            <p className="text-white font-bold text-sm">AgroFarm Bot</p>
                            <p className="text-emerald-200 text-[10px] font-medium uppercase tracking-wider">
                        ...
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                                ...
                                <div className={`max-w-[80%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed ${
                                    ...
                                    <p className={`text-[9px] mt-1 ${msg.role === "user" ? "text-emerald-200" : "text-gray-400"}`}>
                        ...
                        {loading && (
                            <div className="flex gap-2 items-start">
                                ...
                                <div className="bg-white px-4 py-3 rounded-2xl rounded-bl-md shadow-sm border border-gray-100">
                                    <div className="flex gap-1.5"><div className="w-2 h-2 bg-emerald-400 rounded-full animate-bounce" /><div className="w-2 h-2 bg-emerald-400 rounded-full animate-bounce" style={{ animationDelay: "0.15s" }} /><div className="w-2 h-2 bg-emerald-400 rounded-full animate-bounce" style={{ animationDelay: "0.3s" }} /></div>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="px-4 py-2 flex gap-2 border-t border-gray-100 bg-white">
                            {quickActions.map(a => (
                                <button key={a.label} onClick={() => sendMessage(a.msg)}
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 text-[11px] font-semibold hover:bg-emerald-100 transition-colors cursor-pointer">
                    ...
                    <div className="px-3 py-3 bg-white border-t border-gray-100">
                        <form onSubmit={e => { e.preventDefault(); sendMessage(input); }} className="flex items-center gap-2">
                            <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
                                ...
                                className="flex-1 px-4 py-2.5 bg-gray-100 rounded-xl text-sm outline-none focus:ring-2 focus:ring-emerald-200 transition-all"
                                disabled={loading} />
