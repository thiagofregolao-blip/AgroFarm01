...
import { Plus, FileX, Trash2, AlertTriangle, X, Check } from "lucide-react";
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function CreditNotesEmission() {
    ...
    const { data: creditNotes = [] } = useQuery<CreditNote[]>({
        queryKey: ["/api/farm/credit-notes", "emission"],
        queryFn: () => fetch("/api/farm/credit-notes?type=emission").then(r => r.json()),
    });

    const { data: availableInvoices = [] } = useQuery<AvailableInvoice[]>({
        queryKey: ["/api/farm/credit-notes/invoices/available", "receivable"],
        queryFn: () => fetch("/api/farm/credit-notes/invoices/available?invoiceType=receivable").then(r => r.json()),
        enabled: showForm,
    });

    const firstLinkedId = linkedInvoices[0]?.invoiceId;
    const { data: receivableItems = [] } = useQuery<ReceivableItem[]>({
        queryKey: ["/api/farm/credit-notes/receivable-items", firstLinkedId],
        queryFn: () => fetch(`/api/farm/credit-notes/receivable-items/${firstLinkedId}`).then(r => r.json()),
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            toast({ title: "Nota de crédito emitida com sucesso" });
            qc.invalidateQueries({ queryKey: ["/api/farm/credit-notes"] });
            qc.invalidateQueries({ queryKey: ["/api/farm/accounts-receivable"] });
            resetForm();
        },
    ...
    function resetForm() {
        setShowForm(false);
        setClient(""); setTimbrado(""); setNoteNumber("");
    ...
    function addReturnItem(inv: ReceivableItem) {
        setItems(prev => [
            ...prev,
            {
    ...
    function handleSubmit() {
        if (!client.trim()) return toast({ title: "Informe o cliente", variant: "destructive" });
    ...
    return (
        ...
        <div className="p-4 md:p-6 max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-xl font-bold text-gray-900">Emissão de Notas de Crédito</h1>
                    <p className="text-sm text-gray-500 mt-0.5">Notas de crédito emitidas aos seus clientes</p>
                </div>
            ...
            {creditNotes.length === 0 ? (
                <div className="text-center py-16 text-gray-400">
                    ...
                    <p className="text-sm">Nenhuma nota de crédito emitida</p>
                </div>
            ) : (
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {creditNotes.map(cn => (
                                <tr key={cn.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 font-medium">{cn.client}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                        ...
                        <p className="text-sm text-gray-600 mb-6">
                            A nota será anulada e os saldos em contas a receber serão restaurados.
                ...
                <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 overflow-y-auto py-8 px-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
                        <div className="flex items-center justify-between px-6 py-4 border-b">
                            <h2 className="font-bold text-gray-900">Nova Nota de Crédito — Emissão</h2>
                        ...
                        <div className="px-6 py-5 space-y-6">
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        >
                                            <option value="discount">Desconto ao Cliente</option>
                            ...
                            {/* Faturas vinculadas */}
                            <section>
                                <h3 className="text-xs font-semibold uppercase text-gray-400 mb-3">Faturas Emitidas Vinculadas *</h3>
                                <Input
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    {noteType === "return" && (
                                        <span className="text-xs text-gray-400">
                                            {linkedInvoices.length === 0 ? "Vincule uma fatura para carregar itens" : `${receivableItems.length} item(s) disponíveis`}
                                        </span>
                                    )}
                                </div>

                                {noteType === "return" && receivableItems.length > 0 && (
                                    <div className="mb-3 border rounded-lg overflow-hidden">
                                        ...
                                        {receivableItems.map(ri => (
                                            <div
                                                key={ri.id}
                                                onClick={() => addReturnItem(ri)}
                                            ...
                                            >
                                                ...
                                                <span className="text-gray-400 text-xs">{fmt(ri.unitPrice)}/{ri.unit || "un"} · qtd {ri.quantity}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                        ...
                        <div className="px-6 py-4 border-t flex justify-end gap-3">
                            ...
                            <Button onClick={handleSubmit} disabled={createMutation.isPending || !balanced}>
                                {createMutation.isPending ? "Emitindo..." : "Emitir Nota de Crédito"}
                            </Button>
                        </div>
                    </div>
                </div>
