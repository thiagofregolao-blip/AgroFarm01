...
// OneSoil-style NDVI palette — vivid red→orange→yellow→lime→green→dark green
// Matches the spectral coloring used by professional agricultural monitoring platforms
...
function evaluatePixel(sample) {
  ...
  // Mask: no data, saturated, cloud shadow, cloud high/medium/thin, snow, water
  if ([0, 1, 3, 8, 9, 10].includes(sample.SCL)) return [0, 0, 0, 0];

  let ndvi = index(sample.B08, sample.B04);

  // Clamp to valid range
  if (ndvi < -0.2) ndvi = -0.2;
  if (ndvi > 1.0) ndvi = 1.0;

  return valueInterpolate(ndvi,
    [-0.2, 0.0, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.0],
    [
      [0.65, 0.00, 0.00, 1],   // -0.2  dark red (water/bare)
      [0.84, 0.08, 0.04, 1],   //  0.0  red (bare soil)
      [0.92, 0.16, 0.04, 1],   //  0.05 bright red
      [0.97, 0.30, 0.04, 1],   //  0.10 red-orange
      [1.00, 0.44, 0.02, 1],   //  0.15 orange
      [1.00, 0.58, 0.02, 1],   //  0.20 deep orange
      [1.00, 0.70, 0.02, 1],   //  0.25 orange-yellow
      [1.00, 0.82, 0.02, 1],   //  0.30 yellow-orange
      [0.98, 0.92, 0.02, 1],   //  0.35 yellow
      [0.88, 0.96, 0.02, 1],   //  0.40 yellow-green
      [0.60, 0.85, 0.02, 1],   //  0.50 lime green
      [0.30, 0.72, 0.02, 1],   //  0.60 green
      [0.10, 0.56, 0.02, 1],   //  0.70 medium green
      [0.02, 0.42, 0.02, 1],   //  0.80 forest green
      [0.00, 0.30, 0.00, 1],   //  0.90 dark green
      [0.00, 0.20, 0.00, 1]    //  1.0  very dark green
    ]
  );
}`;

// Standard NDVI — same professional palette
...
function evaluatePixel(sample) {
  ...
  return valueInterpolate(ndvi,
    ...
    [
      [0.65, 0.00, 0.00, 1],   // dark red
      [0.84, 0.08, 0.04, 1],   // red
      [0.97, 0.30, 0.04, 1],   // red-orange
      [1.00, 0.58, 0.02, 1],   // orange
      [1.00, 0.82, 0.02, 1],   // yellow-orange
      [0.88, 0.96, 0.02, 1],   // yellow-green
      [0.60, 0.85, 0.02, 1],   // lime
      [0.30, 0.72, 0.02, 1],   // green
      [0.10, 0.56, 0.02, 1],   // medium green
      [0.02, 0.42, 0.02, 1],   // forest green
      [0.00, 0.30, 0.00, 1],   // dark green
      [0.00, 0.20, 0.00, 1]    // very dark green
    ]
  );
}`;
